...
IMPORTANT: The primary observable is std(rv_comp_1), the standard
deviation of the raw spectroscopic radial velocity from GALAH.
This is NOT the same as std(U_UVW), which is a derived quantity.
Each velocity component (U, V, W) has a different base value but
the same sigmoid shape. See Test 4 (Model C) for details.
...
# ============================================================
# NEW: MODEL C — Pure Sigmoid, Zero Heating (Added May 29, 2026)
# ============================================================

def test_model_c(df):
    """The four-number model: sigma = 23.7 * gamma(t)"""
    
    print("\n" + "="*70)
    print("TEST 4: MODEL C — Pure Sigmoid, No Heating")
    print("sigma(t) = 23.7 * (1 + 0.79 / (1 + exp(-0.78 * (t - 7.9))))")
    print("="*70)
    
    from scipy.stats import halfnorm
    
    def gamma(t):
        return 1.0 + 0.79 / (1.0 + np.exp(-0.78 * (t - 7.9)))
    ...
    for pct in percentiles:
        print(f"  P{pct} obs  pred", end="")
    print()
    
    for alo in range(2, 13):
        ag = solar[(solar['age'] >= alo) & (solar['age'] < alo + 1)]
        if len(ag) >= 50:
            t = alo + 0.5
            sigma_pred = 23.7 * gamma(t)
            
            print(f"{t:>5.1f}", end="")
            for pct in percentiles:
                obs = np.percentile(np.abs(ag['rv_comp_1']), pct)
                pred = halfnorm.ppf(pct/100) * sigma_pred
                all_obs.append(obs)
                all_pred.append(pred)
                print(f"  {obs:>6.1f} {pred:>5.1f}", end="")
            print()
    ...
    r2 = 1 - ss_res / ss_tot
    rmse = np.sqrt(np.mean((all_obs - all_pred)**2))
    
    print(f"\n  R² = {r2:.4f}")
    print(f"  RMSE = {rmse:.2f} km/s")
    print(f"  n = {len(all_obs)} data points")
    print(f"  Parameters: 4 (base=23.7, L=0.79, k=0.78, t0=7.9)")
    print(f"  Expected: R² > 0.999")


# Add to main if running standalone
if __name__ == "__main__":
    import sys
    if 'galah_dr4_stars.csv' in [f for f in __import__('os').listdir('.')]:
        df = pd.read_csv('galah_dr4_stars.csv')
        df['absrv'] = abs(df['rv_comp_1'])
        df_met = df[(df['fe_h'] >= -0.3) & (df['fe_h'] <= 0.3) & (df['absrv'] < 150)]
        test_model_c(df_met)
